...
void main()
{
    ...
    float rings = fract(constrain_scaled_distance(distFromCenter, 4., 2., 2. - iTime * 5.)); // expanding rings around explosion
    rings *= rings; // increase contrast

    float timeDimming = 1. - smoothstep(0.5, 2., iTime);
    float brightness = (rings) * distance_dimming * distance_dimming * timeDimming;
